module AmazonMWS
  ...
  class Base
    ...
    def initialize(options ={})
      @connection = AmazonMWS::Connection.connect(options)
    end
    
    def connection
      raise AmazonMWS::NoConnectionEstablished.new if !connected?
      @connection
    end
    ...
    def request(verb, path, query_params = {}, body = nil, attempts = 0, &block)
      # Find the connection method in connection/management.rb which is evaled into AmazonMWS::Base
